...
### 11.3 범위 요약 — 해야 할 것 / 결정 필요 / 제외할 것

§8.4(Non-Goals)·§11.1~11.2·PRD §27(Open Questions)에 흩어진 범위 판단을 하나의 표로 정리한다. 새 결정을 내리지 않고 기존 내용을 재배열한 것이다.

| 구분 | 내용 |
|---|---|
| ✅ **해야 할 것 — 단계 1** | REQ-FUNC-001~021, 024, 025 (§11.1의 FT1~FT7) |
| ✅ **해야 할 것 — 단계 2** | REQ-FUNC-022, 023 (§11.2의 FT8~FT9) |
| 🔶 **결정 필요 (TBD)** | 네이버 정책(관심매물 API 권한, 공유객체 비로그인 열람, 검색 count·필터 파라미터) · 개인정보 공개 범위·관계종료 데이터 처리 · 화면(재탐색 필터 상세, 실연비·통행료 입력 위치) · 지표(North Star 7일 적정성) — REQ-FUNC-007/008/015/016·FR-23에 걸린 TBD와 동일(§4.1, PRD §27) |
| ❌ **제외할 것** | §8.4 Non-Goals 전체(매물DB·종합점수·AI 최종선택·계약/대출 실행·3인 이상 등) + 방문일정조율·별도 공동메모·중개사 문의 실행 자체·초대취소 상세흐름·계약완료 이후 관리(PRD §23.2) |
| ❌ **SRS 문서 범위에서 제외** | KPI/성공 지표, 화면별 인벤토리 — 위 표지의 "확장 챕터" 안내 참고 |
