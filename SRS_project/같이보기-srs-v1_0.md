...
> **근거:** ISO/IEC/IEEE 29148:2018 §9.6.19 Verification — 소프트웨어를 적격화하기 위해 계획된 **검증 접근법과 방법**을 제시하며, §9.6.10 ~ §9.6.18의 정보 항목과 **병렬로** 기술할 것을 권고한다. 본 장은 §4의 요구사항과 병렬로 성과 지표·가설 검증·관측 항목·릴리스 게이트를 기술한다.
...
PRD는 무작위 배분 A/B 실험 대신 **가설(H1~H5) 대비 실측·인터뷰** 방식을 채택한다(PRD §25) — 비교군(결혼준비 앱 등)의 실제 수치가 비공개라 절대 기준선을 세울 수 없기 때문이다(`03-evidence-log.md`). 아래는 실험 설계가 아니라 **검증 설계**이며, 존재하지 않는 표본 크기·기간을 지어내지 않는다.

| ID | 검증 대상 가설 | 검증 방법 | 통과 기준 | 거짓일 때 | 실행 순서 |
| --- | --- | --- | --- | --- | --- |
| **H1** | 초대받은 상대가 들어와 조건을 입력한다 | 3단 깔때기(진입→입력→로그인) + 예비부부/룸메이트 집단 간 비교 | 절대 목표 없음 — 집단 간 상대 비교 | 2인 전제 재검토, 개선 실험 2회 실패 시 `decisions/0002` 되돌림 검토 | 1 (선행) |
| **H2-b-2** | 조건 충돌이 실제 결정을 지연시킨다 | 완전공동형 경험자 인터뷰, 지연 원인을 조건 충돌/일정 조율/비용 분담으로 구분 | 지연 경험 과반 + 조건 충돌이 다른 원인보다 많이 지목 | Problem Statement 재작성 | 2 (H1과 병행 가능) |
| **H3** | 사용자가 조건 입력을 감수한다 | 단계별(예산→출퇴근→추가조건) 입력 완료율 측정 | 최신 입력 단계별 완료율 | 추가 입력 범위 재검토 | 4 (H1 성립 후) |
| **H4** | 보유 데이터로 의미 있는 비용 비교가 가능하다 | 전제 문구 공개 전후 신뢰도 비교 | 전제 공개 후 신뢰도 상승 | 절대 추정 대신 상대 차액 중심으로 후퇴 | 5 (H3과 병행) |
| **H5** | 매물 회전이 비교 주기보다 느리다 | 후보 등록→확정 중앙값 vs 매물 평균 게시 기간 비교 | 중앙값 < 게시 기간 | 매물 수명과 조건 지속 구조 재검토 | 5 (H4와 병행) |

**H2-b-2 인터뷰 조건**(PRD §25.3): 최근 6개월 내 2인 이상 완전공동형(주도–승인형 제외) 경험자, 조건 충돌·일정 조율·승인 단계·예산 합의를 지연 원인으로 구분해 질문하며, "둘이 구하면 오래 걸린다"는 일화를 사실로 전제하지 않는다.
...
| §10.2 가설 검증 · §10.4 게이트 | §25 Validation Plan / Open Hypotheses |
