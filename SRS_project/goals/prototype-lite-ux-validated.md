...
- **디렉터리 레이아웃은 `SRS_V0_9.md` §2.2를 따른다** — `src/` 를 쓰지 않고 저장소 루트에 `app/`, `components/`, `lib/`, `prisma/` 를 둔다. `create-next-app` 실행 시 `--no-src-dir` 를 지정한다.
...
- **기록 — `reports/PROTOTYPE_LOG.md` 를 작업 시작 시 만들고 아래를 append 한다:**
...
- **종료 조건 (아래 중 하나라도 충족되는 순간 루프를 즉시 멈춘다):**
  ...
  - `aztks-agent` EVALUATE 디스패치 누적 5회 도달 → **STOP REASON: EVAL_BUDGET**
...
- **활성 범위 외 변경 금지.** 단 `reports/` 와 `.claude/` 는 예외로 허용한다.

## 5) aztks-agent 평가 규칙

- **디스패치 형식** — Task 도구로 `subagent_type: "aztks-agent"` 를 호출하고 프롬프트 첫 줄에 `MODE: EVALUATE` 를 넣는다. 평가는 **읽기 전용**이며 코드를 수정하지 않는다.
- **평가 대상:** 구현된 프로토타입 전체(`app/`, `components/`, `lib/`)와 `reports/prototype-screens/` 의 화면 캡처 32장.
- **근거 소스로 함께 전달한다:**
...
- **5축 판정 앵커** — 각 축을 아래 근거로만 판정하게 한다:
  ...
  | **K 깔끔** | `lib/dev/` 경계가 유지되는가(`lib/domain/`·`lib/queries/`에 섞이지 않음). 화면이 `lib/types/contracts.ts` 외 타입을 스스로 선언하지 않는가. 픽스처 주입 지점이 화면당 정확히 1곳인가 |
...
- `S-01`(로딩)·`S-03`(경로 없음)은 캡처 대상에서 제외한다 — `S-01`은 순간 상태라 재현이 불안정하고, `S-03`은 PRD §18.3상 `S-02`와 동일한 `계산 불가` 처리라 32번이 대표한다.
