...
옵션 B(기능 Task당 테스트 companion) 채택 — **확정 산출(상세 `.md` 작성 완료) 60개**(기능/인프라 36개 + 테스트 companion 24개) + **Macro 산출(목록만 확정, 상세 대기) 6개**(NFR, Phase 10). Macro→Micro 2단계 파이프라인(안티패턴: 한 프롬프트로 전체 상세를 한 번에 뽑는 것)에 따라, NFR 6개는 이번에 목록만 확정하고 상세는 이후 1~3개씩 별도 프롬프트로 진행한다.
...
## Phase 10 (NFR) — Macro 단계 산출물, 상세 미작성

> ⚠️ 아래 6개는 **Macro 단계 산출물이다.** Phase 1(숲을 보는 프롬프트) 형식의 목록만 확정했으며, Phase 2(나무를 심는 프롬프트)에 따라 **한 번에 1~3개씩** 개별 `.md` 파일을 작성한다. 이 표만으로는 아직 착수할 수 없다.

기존 36개 기능 Task 어디에도 다뤄지지 않은 REQ-NF 항목(REQ-NF-011은 X-004가 이미 커버 — 중복 아님, 제외)을 인프라·보안·성능 Task로 추출했다.

| Task ID | Epic (도메인) | Feature (기능명) | 관련 SRS 섹션 | 선행 Task (Dependencies) | 복잡도(H/M/L) |
| --- | --- | --- | --- | --- | --- |
| N-001 | 성능(Performance) | E2E 응답시간 P95≤30초 부하 테스트 스크립트 작성 | `같이보기-srs-v1_0.md` §4.2 REQ-NF-001, `같이보기-technical-design-v1_0.md` §8 성능 예산 배분 | D-001, S-003 | M |
| N-002 | 성능/확장성(Scalability) | 경로 API 캐시(`route_cache`) 히트율 부하 테스트, 완화 재계산 시 재호출 0회 검증 | REQ-NF-002·003 | R-002, C-000 | M |
| N-003 | 안정성(Reliability) | 네이버 API 3종 장애 주입 테스트 — 실패 시 `CALCULATION_FAILED` 폴백 검증 | REQ-NF-004 | C-002 | M |
| N-004 | 보안(Security) | Supabase RLS 접근 제어 정책 — 초대된 두 사람만 열람 가능 | REQ-NF-005 | C-000, A-001 | H |
| N-005 | 정확성/코드품질 | `DisclosedValue` 미경유 숫자 렌더링을 빌드 타임에 차단하는 ESLint 커스텀 규칙 | REQ-NF-006 | X-001 | M |
| N-006 | 유지보수성(Maintainability) | 신규 `ConditionType` 추가가 레지스트리 등록 1줄로 가능한지 검증하는 확장성 회귀 테스트 | REQ-NF-007 | J-003~007 | L |

**복잡도 판단 근거:** N-004(RLS)만 High로 매겼다 — Supabase RLS 정책은 정책 조건을 잘못 쓰면 "전혀 안 보임"과 "전부 다 보임" 양쪽으로 조용히 실패하는 경우가 흔해, 다른 5개(테스트/린트 스크립트 작성 위주)보다 검증 부담이 크다.
