...
3. 즉, "표준을 준수하기 위해 문서를 부풀리는 것"이 아니라 "예시 문서의 포맷을 기준으로 PRD를 옮기고, 모자라는 부분만 표준 구조에 맞춰 보충하는 것"이 이번 작업의 정확한 목표다. `ai-place-srs-v1_0.md`의 §1~§7은 예시 템플릿과 같은 포맷이고, §8~§12는 그 확장이다(각 장이 대응하는 §9.6 조항을 본문에 명시했다).
...
```
PRD (ai-place-prd-v1_0.md)
        │  위 "SRS 작업 목표" 원칙에 따라 변환
        ▼
SRS (ai-place-srs-v1_0.md) — §1~7 템플릿 포맷 그대로 + §8~12 표준 기반 확장
        │
        ▼
...
| 경로 | 내용 |
| --- | --- |
| `SRS_project/ai-place-prd-v1_0.md` / `.html` | PRD v1.0 (마크다운 원본 / 다이어그램 포함 HTML 렌더링본) |
| `SRS_project/ai-place-srs-v1_0.md` / `.html` | PRD 기반 SRS 문서 (마크다운 원본 / HTML 렌더링본) |
