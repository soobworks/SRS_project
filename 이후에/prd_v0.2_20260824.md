...
> **v0.2 — PRD 완성도 재검토 반영본**  
> `prd_v0.1_20260820_with_AC.md`(AC 확장본)의 본문과 FR-01~FR-25 요구사항은 그대로 두고, 2026-08-24 자체 검토에서 미흡으로 지적된 4개 항목 — KPI 수치화 프로토콜(§24.4), MoSCoW·스프린트 구현 가능성(§16.2), 비기능요구(§20), 엔터티 관계도(§21.9) — 를 추가했다. 근거는 우선 이 폴더의 handoff 원본(`_handoff_extracted/`)과 `Day4_KPI_설계_보강(가드레일_포함).md`, `docs/04-benchmarks.md`에서 찾아 반영했고, 원본에 근거가 없는 항목은 새 수치를 지어내지 않고 `[제안 — 팀 확정 필요]`로 표시했다. 상세 변경 로그는 문서 끝 "PRD 완성도 재검토 (v0.2)" 참고.
...
| 버전 | `v0.2` (AC 확장본 `v0.1` 기반, 비기능요구·MoSCoW·KPI 프로토콜·엔터티 관계도 추가) |
...
| 작성 기준일 | 2026-08-20 (v0.2 개정 2026-08-24) |
...
| 문서 상태 | 구현 전 `v0.2`. 미결정 사항과 외부 의존성은 `[검증 가설]`·`[TBD]`로 유지한다. |
...
| FR-03 | Should | 점진적 확장은 H3 완화 설계의 핵심이나, FR-02만으로도 첫 판정은 성립한다 | FR-10 | 가능 |
| FR-04 | Should | 선호·확인 분리는 Hero Feature 품질에 기여하나 판정 자체를 막지 않는다 | FR-10 | 가능 |
...
| FR-10 | **Must** | 판정 엔진 자체 — 없으면 Hero Feature와 North Star 모두 성립하지 않는다 | FR-01, FR-02 | **아니오 — 분할 필요.** 상한형(예산) 판정만 먼저 구현하는 티켓(FR-10a)으로 시작하고, 하한형·유무형·일치형까지의 확장 판정(FR-10b)은 다음 스프린트로 분리한다 |
| FR-11 | Must | 3분류 그룹이 없으면 사용자가 어디부터 봐야 할지 알 수 없다 | FR-10 완료 | 가능 |
| FR-12 | **Must** | 양보 문장이 이 제품의 존재 이유다(`decisions/0001`) | FR-10 완료 | 조건부 — 문장 생성 로직 자체는 1스프린트 내 가능하나 FR-10 선행 완료가 전제 |
| FR-13 | Should | 완화 미리보기는 H2-b 검증에 필요하나 판정 자체보다 후행 기능이다 | FR-10 | 가능 |
...
| FR-18 | Should | 조건 지속은 2~3회 반복 세션(H5)에서만 체감되는 가치다 | FR-10 | 가능 |
| FR-19 | Should | 매물 소진은 실제 발생 전까지는 상대적으로 저빈도인 예외다 | FR-17 | 가능 |
| FR-20 | Must | 상태 오분류(미충족/계산 불가/확인 필요/해당 없음) 방지가 곧 판정 신뢰도다 | FR-10 | 가능 — FR-10과 함께 구현 |
| FR-21 | Must | 숫자 전제 공개는 H4 검증과 신뢰 리스크 대응의 최소 조건이다 | FR-10 | 가능 |
...
| 매물 자동 판정(FR-10) 재계산 | `[제안]` 조건·후보 변경 후 다음 사용자 조작 전에 결과 반영 | FR-10 AC-10-01 | 판정 실패·타임아웃 건수 |
...
| 스토리·AC(GWT, SLO, 실패 케이스 2개 이상) | 미충족 — GWT 형식·SLO 없음 (v0.1 AC 확장본에서 일부 보완됨) | 이번 v0.2에서는 추가 변경 없음. FR별 AC는 `with_AC.md`에서 상속했고 실패 케이스는 각 AC 안에 서술형으로 포함되어 있으나 정식 GWT 표기는 아님 | §16.1 (기존) | 변경 없음 — 남은 과제로 유지 |
| 기능 요구(MoSCoW·근거·의존성, 1스프린트 내 구현 가능성) | 부분 충족 — MoSCoW 미사용, 스프린트 단위 평가 없음 | FR-01~25 전체를 MoSCoW로 매핑하고 의존성·1스프린트 구현 가능 여부를 표로 명시. FR-10처럼 1스프린트를 넘는 항목은 하위 티켓 분할을 제안 | §16.2 | §5, §15, `decisions/0001~0004` (기존 결정을 재인용) |
...
**남은 과제** — 스토리·AC를 정식 Given-When-Then 표기로 재작성하는 것은 이번 v0.2에 포함하지 않았다. 기존 `with_AC.md`의 서술형 AC 56개를 GWT 3단으로 재구조화하는 작업은 별도 라운드로 남긴다.
