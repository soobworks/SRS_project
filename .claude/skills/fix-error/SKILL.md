...
when_to_use: Use when the user reports an error, exception, stack trace, or failing test and wants a structured root-cause diagnosis rather than a guess-and-patch fix.
---
...
## 이 프로젝트 특화 확인
- 판정(`domain/judgment/`) 관련 버그라면 §3 절대 규칙(총점 금지, 4상태 오분류 금지)을 먼저 의심한다 — `judgment-engine` 서브에이전트로 위임을 고려한다.
